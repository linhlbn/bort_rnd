...
async def embedding_process(files_dict, CI, CR, CP, CO, chosen_categories, timer, stop_event):
                ...
                for idx, file in enumerate(filenames, start=1):
                    ...
                    if doc_code not in existing_doc_codes:
                        ...
                        try:
                            ...
                            print(f"||||  Congratulation!!! File {file} has been embedded successfully with doc_code {doc_code} at {current_time}!")
                            ...
                            if "rate_limit_exceeded" in str(e):
                                ...
                                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                print(f"\n The program has run {count} times at {current_time}!")
                                is_stop = True
                            else:
...
async def periodic_run(interval, async_func, *args):
    ...
    while not stop_event.is_set():
        await async_func(*args, stop_event) 
